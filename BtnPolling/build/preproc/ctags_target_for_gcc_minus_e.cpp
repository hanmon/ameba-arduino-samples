# 1 "d:\\project\\ameba-arduino-samples\\BtnPolling\\BtnPolling.ino"
/*
...
*/
# 11 "d:\\project\\ameba-arduino-samples\\BtnPolling\\BtnPolling.ino"
const byte btnPin = 3;
const byte ledPin = 9;
const int debounceTime = 20;//設定除跳時間

void setup() {
  pinMode(btnPin, 0x00);//設定按鍵腳位為輸入
  pinMode(ledPin, 0x01);//設定LED腳位為輸出
  Serial.begin(115200);//設定序列埠速率
  Serial.println("Button pressed detection");//印出提示字串
}

void toggleLED() {
  static boolean ledState = 0x0;//宣告static變數以儲存led狀態
...
void debounceMethodOne() {
  static boolean lastBtnState = 0x0;//宣告static boolean變數以儲存最新的按鍵偵測狀態
  if (lastBtnState!=digitalRead(btnPin)) {//若狀態有變遷
    ...
    if (lastBtnState!=digitalRead(btnPin)) {//若除跳後狀態維持
      ...
      if (lastBtnState == 0x0) {//若按鍵偵測狀態為LOW，代表使用者按下按鍵又放開，因此判定為一次按下事件
        toggleLED();//切換LED燈狀態
...
void debounceMethodTwo() {
  ...
  if (lastBtnState != digitalRead(btnPin)) {//若狀態有變遷
    ...
    if (lastBtnState == 0x0) {//除跳結束，若按鍵偵測狀態為LOW，代表使用者按下按鍵又放開，因此判定為一次按下事件
      toggleLED();//切換LED燈狀態
      Serial.println("Led Toggled");
    }
  }
